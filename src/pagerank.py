...
def sousGraphe(model,query,k,n):
    #pour la requête query la méthode détermine le sous graphe  des documents candidats sur lequel nous allons lancer un algorithme de marche aléatoire
    """
    k : int , represente le nombre des documents choisis aléatoirement parmi ceux pointant vers un document D
    n : int , represente le nombre des premiers documents retournés par un modéle sur le contenu donné
    """
...
def pagerank(model,query,d,k,n,eps,nbiter):
    #applique l’algorithme de PageRank et ordonner les documents selon leur l’importance
    """
    k : int , represente le nombre des documents choisis aléatoirement parmi ceux pointant vers un document D
    n : int , represente le nombre des premiers documents retournés par un modéle sur le contenu donné
    d : float , le facteur d’amortissement
    """    
...
def optimisationD(model,start1,end1,n1,query,k,n,eps,maxiter,graph=False):
    #la méthode permet de faire une recherche de valeur optimale du facteur d’amortissement d par GridSearch.    
